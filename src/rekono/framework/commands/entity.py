...
from rekono.framework.arguments import id_mandatory_argument, id_optional_argument
...
class EntityCommand(ApiCommand):
    ...
    def extra_post_entity(
        ...
        ctx.invoke(
            EntityCommand.post,
            endpoint=f"/api/{cast(click.Context, ctx.parent).info_name}/{id}/{ctx.info_name}/",
            url=url,
            headers=headers,
            no_verify=no_verify,
            body=None,
