...
class RekonoCliCommand(click.MultiCommand):
    ...
    def _get_body(body: Optional[str]) -> Optional[str]:
        ...
        if body:
            try:
                ...
                click.echo(
                    click.style("Invalid JSON format for body value", fg="red"),
                    err=True,
                    color=True,
                )
                sys.exit(1)
    ...
    @classmethod
    def _rekono_factory(
        cls, url: str, no_verify: bool = False, headers: List[str] = []
    ) -> Rekono:
    ...
    def _display_responses(
        ...
        if quiet:  # No content should be displayed
            return
        if only_show_status_code or show_headers:
            # Headers or status should be displayed
            for response in responses:  # For each response
                if only_show_status_code:  # Just display status code
                    ...
                    click.echo(
                        f"{response.request.method} {response.request.path_url} {response.status_code}"
                    )
                    ...
                    data = cls._get_data_from_responses([response])
                    # Display content via standard output
