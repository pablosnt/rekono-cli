...
class AuthenticationsCommand(EntityCommand):
    ...
    entity_options = [
        click.option(
            ...
            required=True,
            type=int,
            help="Related target port ID",
        ),
        click.option(
            "-n", "--name", "name", required=True, type=str, help="Authentication name"
        ),
        click.option(
            "-c",
            "--credential",
            ...
            required=True,
            type=str,
            help="Authentication secret value",
        ),
        click.option(
            "-t",
            "--type",
            "auth_type",
            required=False,
            default=AuthenticationType.BASIC.value,
...
@click.group("authentications", cls=AuthenticationsCommand)
def authentications():
