...
class WordlistsCommand(EntityCommand):
    ...
    entity_options = [
        click.option(
            "-n", "--name", "name", required=True, type=str, help="Wordlist name"
        ),
        click.option(
            "-t",
            "--type",
            "type",
            required=False,
            default=WordlistType.ENDPOINT.value,
...
@click.group("wordlists", cls=WordlistsCommand)
def wordlists():
    """Manage wordlists."""
